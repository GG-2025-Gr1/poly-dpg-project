...
class ProductionP4(Production):
    ...
    def find_lhs(
        ...
        for _, data in graph._nx_graph.nodes(data=True):
            ...
            if self.DEBUG:
                print(
                    f"[P4] - znalezione wierzchołki hiperkrawędzi: {hyperedge_vertices}"
                )
    ...
    def apply_rhs(self, graph: Graph, match_node: Hyperedge):
        ...
        if self.DEBUG:
            print(
                f"[P4] Dzielę krawędź {match_node.uid} między wierzchołkami {v1.uid} i {v2.uid}"
            )
        ...
        max_vertex_id = max(
            [
                int(re.search(r"\d+", str(node_id)).group())
        ...
        new_vertex = Vertex(uid=new_vertex_id, x=mid_x, y=mid_y, hanging=True)
        graph.add_vertex(new_vertex)

        if self.DEBUG:
            print(
                f"[P4] Utworzono nowy wierzchołek {new_vertex_id} w ({mid_x}, {mid_y})"
            )
        ...
        print(
            f"-> P4: Podzielono krawędź brzegową {match_node.uid}, utworzono wierzchołek {new_vertex_id} i krawędzie {edge1_id}, {edge2_id}."
        )
